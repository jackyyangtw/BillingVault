...
export default function NewPaymentMethodOption({
  ...
  return (
    <button
      type="button"
      className={cn(
        "hover:bg-muted/40 grid gap-4 rounded-3xl border p-4 text-left transition-colors md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-center",
        isSelected &&
    ...
    >
      <div className="bg-card/80 ring-foreground/10 dark:bg-foreground/5 dark:ring-foreground/10 flex size-16 items-center justify-center rounded-2xl shadow-sm ring-1">
        {hasPaymentMethods ? <Plus /> : <CreditCard />}
      </div>
